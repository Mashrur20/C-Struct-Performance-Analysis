...
int
processFasta(char *filename, double *timeTaken, LLvNode * llHead, LLvNode * llTail)
{
	FILE *fp;
	FASTArecord fRecord;
	...
	LLvNode * block = NULL;//creating a LLvNode pointer and assigning it to NULL
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			lineNumber += status;
			block = llNewNode(fRecord.description,(void*)fRecord.id);//calling the NewNode function to create a new node
            if(llHead == NULL){
				llHead = llTail = llAppend(llHead, block);//if the head is NULL which is the list is empty then the new node is added at the end
			}
			else{
				llTail = llAppend(llTail, block);//if there are data in the head then the new node is added to the end of llTail
				llTail = llTail->next;
			}
			free(block);//freeing the node so that there are no memory leaks
			recordNumber++;
	...
	)
{
	...
	for (i = 0; i < repeatsRequested; i++) {
		status = processFasta(filename, &timeThisIterationInSeconds, llHead, llTail);
	 ...
	 for (i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			...
			recordsProcessed = processFastaRepeatedly(argv[i],
					repeatsRequested, llHead, llTail);
