...
int
processFasta(char *filename, double *timeTaken, LLvNode * llHead)
{
	FILE *fp;
	FASTArecord fRecord;
	...
	LLvNode * buffer = NULL;// creating a LLvNode pointer and assigning it to NULL
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			lineNumber += status;
			buffer = llNewNode(fRecord.description, (void*)fRecord.id);// calling the NewNode function to create a new node
			llHead = llPrepend(llHead, buffer);//adding the new node to the beginning of llHead
			free(buffer);//freeing the node so there are no memory leaks
			recordNumber++;
	...
	)
{
	...
	for (i = 0; i < repeatsRequested; i++) {
		status = processFasta(filename, &timeThisIterationInSeconds, llHead);
